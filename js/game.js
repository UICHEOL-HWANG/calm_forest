...
const workerObjs = [];          // { rec, group, armR, armL, hat, tool, task, plot, phase, t }
let workerStepAcc = 0;          // 밭 밖에서 쌓이는 초 — 60초마다 오프라인 스텝 1회
const WORKER_SPEED = 1.35;      // 걷는 속도(🐿️장인은 ×1.5)

function workerCap() { return (FARM_STAGES.find(s => s.stage === (gameState.farm.stage || 1)) || FARM_STAGES[0]).workers; }
function hasHireBoard() { return farmBuildingRecs().some(b => b.id === 'board'); }
function warehouseTotalCap() { return warehouseCap(farmBuildingRecs()); }
function pendingTotal() { return storageTotal(gameState.farm.pending); }
function shelterNear(x, z) { return inRadiusOf(farmBuildingRecs(), 'shelter', x, z); }
...
function farmCellMax(H) { return 2 * Math.floor((H - 1) / 2); }          // 울타리 안 마지막 짝수 칸
function farmCellCount(H) { return Math.pow(farmCellMax(H) + 1, 2); }    // 텃밭 25 · 넓은 밭 81 · 대농장 121칸
function freeFarmCell() {
...
function workerWorld(cellCache) {
  const now = clock.elapsedTime;
...
function workerApply(rec, task, tally) {
...
function workerDidWork(rec, tally) {
...
function workerSteps(steps, tally) {
...
function settleWorkerWages(tally) {
...
function setWorkersVisible(v) { for (const o of workerObjs) o.group.visible = v; }

// 일꾼 이동 — 주민과 달리 밭을 피하지 않는다(밭으로 들어가야 하니까, 스펙 §3-8)
function stepWorkerToward(o, target, speed, dt) {
...
// ── 📋 일꾼 게시판 — 고용 ────────────────────────────────────
const SKILL_LABEL = { water: '💧물주기', till: '⛏️밭 갈기', fert: '🌱비료', weed: '🌿김매기', harvest: '🌾수확', plant: '🌰심기', haul: '🧺운반', pest: '🐛해충 쫓기' };
function gradeSkillText(job, grade) { return skillsOf(job, grade).map(k => SKILL_LABEL[k] || k).join(' · '); }
...
export {
  ...
  SEVERE_TODAY, SEVERE_TOMORROW, WEATHER, _camLook, _camTarget, _hintAnyPrev, _seaPrevTool, _v, analog, applyCosmetics,
  applyHouseStyle, armWristK, atCafe, atFarm, atMine, atMist, atMuseum, atOrchard, atRiver, atSea, awardBadge,
  baitActive, biteAt, biteEnd, blockIfLocked, boat, boatView, bobber, buffOn, buffs, bugJarMesh, bugRespawnAt,
  cafeGuestCache, cafeGuestFetcher, cafeGuestObjs, cafeInGroup, camera, castPos, catchCeremony, clayMat,
  clearCrop, clearPest, clock, colliders, cookTier, cosmeticShop, cropMini, currentTool, cycleSapSel, dateHash,
  decorGhost, decorMeshes, decorNearRing, decorRot, decorTapHintShown, decorTarget, dexDiscover, diffParams,
  disposeTree, dist2D, doPlayerAction, dockGroup, duelFetcher, easeOutBack, farmActionFirst, farmBuildingRecs,
  farmCropMeshes, farmGroup, farmHalf, farmSoilMesh, fertTarget, finishPetJob, firstHint, firstHintBanner,
  fishDiff, fishMesh, fishState, floatTexts, forageNodes, forecastLine, forestGroup, gambrelRoofSlabs, gambrelSolid,
  gameState, ghostOutdoor, giveReward, gladeBugs, gladeGroup, habitatCells, habitatCtx, habitatEnvAt, handAnchor,
  harvestTexture, heldGroup, heldToolMesh, houseCollider, houseFloor, houseGhost, houseGroup, houseSign,
  houseSignCtx, houseSignTex, houseWindows, indoor, interiorFloor, interiorFloors, interiorGroup, interiorLamp,
  isBlocked, isNight, keys, kitchenFinish, kitchenStart, lastDoorPrompt, lastFloorChoiceKey, lastNearHouse,
  lastNearMiss, lastZoneHint, makeCharacterPreview, makeNameTag, makeSignBoard, makeSignpost, mapLocked,
  markHabitatDirty, measureStowLen, mergeGeos, mgView, mineGroup, mineTorches, mist, mistGroup, mistLanterns,
  mistTree, museumGroup, nearBench, nearBoat, nearBoatShop, nearCafeBoard, nearCafeGuest, nearCoop, nearCosShop,
  nearDecorMesh, nearDoor, nearDoorFloor, nearForest, nearGlade, nearKitchen, nearMarket, nearNPC, nearOutdoorMesh,
  nearRank, nearShop, nearStation, nightFetcher, nightLevel, nightNoteFetcher, noteSpecialExhibit, npcObjs,
  obstacles, onPlotArea, orchardSlotsWorld, orchardStreamWorld, oreRocks, outdoorMesh, outdoorMeshes, outdoorTarget,
  paintGeo, pantryHas, pantryTake, pendingDish, pestTarget, pestTexture, petChoresNear, petJob, pickedDecor,
  pickedOutdoor, placeOutdoor, placingDecor, placingOutdoor, player, playerAnchor, playerArms, playerInYard,
  plots, pointer, poseHeldTool, priceOf, priceRate, questEvent, raycaster, rebuildInteriorFinish, rebuildOrchard,
  refreshCollectQuests, refreshCropStage, refreshHeldTool, refreshInventoryUI, refreshStations, removeSolid,
  renderer, respawnPet, rewardText, riverActive, riverCourse, riverGroup, riverPool, rollDifficulty, roundRect,
  scene, seaBuoy, seaFishes, seaGroup, seaLine, seaMG, seaRodMesh, seedHintTexture, setFogExempt, setHeldDecor,
  setHeldTool, setSpaceVisible, settleDifficulty, settleOrchard, severeOf, shared, showCatchItem, sitting,
  situation, snapCamera, solidBox, solidCircle, spawnConfetti, spawnDust, spawnFloatText, spawnLeafBurst,
  spawnSparkle, spawnSplash, spawnTree, spawnWater, spawnWoodChips, stopOutdoorPlacing, swayables, syncBadges,
  syncFarmHints, syncStory, todayStr, toolMesh, toolPage, trackGateBlocked, trees, triggerFarmReveal, triggerMoment,
  tryUnlockDrop, ui, updateCarveScene, updatePlotVisual, updateStowPose, updateToolPageAuto, usePet, vtxMat,
  wantAction, warnTexture, weedTexture, wiltPlot, woodMat, workerCap, worldGround, worldGroundPatches,
};
