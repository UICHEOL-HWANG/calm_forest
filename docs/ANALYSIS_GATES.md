...
> 2026-09-04 갱신 — 추론 위치를 **JS 계수 → VM FastAPI** 로 변경. 서빙 파이프라인을 직접 만드는 것이
> 이 프로젝트의 목적이라서다. 운영상 최선은 Cloudflare Worker 였고, 아파지면 그리로 옮긴다(설계서 §6).
...
2. 그다음 구현 — `js/predict.js` · `ml/api/churn.py` · `ml/train_churn.py` · `infra/airflow/dags/churn_train.py` · `04_churn_model.ipynb`
