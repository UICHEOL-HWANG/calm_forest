...
- [ ] **Step 5: 두 곳에 흩어진 판매가를 테스트로 못 박는다**

`tests/orchard.test.mjs` 에 더한다:

```js
test('FRUITS[].price 는 SELL_PRICE 와 손으로 맞춘 값이다 — 바꾸면 양쪽을 같이 바꾼다', () => {
  assert.deepEqual(
    Object.fromEntries(FRUITS.map(f => [f.id, f.price])),
    { apple: 5, pear: 6, peach: 8, persimmon: 10, chestnut: 12 },
  );
});
```
...
**Files:**
- Modify: `js/game.js:499`(좌표), `js/game.js:302`·`js/game.js:2449`(플래그), `js/game.js:1688`(장소 이름), `js/game.js:1942`(`mapLocked`), `js/game.js:1065`(세이브 초기값), `js/game.js:9808` 부근(입구 프롬프트), `js/game.js:10040`(지도 지명), `js/game.js:524`(가시성)
...
function enterOrchard() {
  ...
  settleOrchard();                                 // 하루가 지났으면 여기서 정산(Task 8)
