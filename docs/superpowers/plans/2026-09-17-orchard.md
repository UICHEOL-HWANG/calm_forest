...
- 🚨 **`js/game.js` 를 고쳤으면 파싱을 확인한다. `node --check js/game.js` 를 믿지 않는다** — Task 5 에서 구현자와 리뷰어가 각각 확인했다. 중괄호를 일부러 안 닫은 손상 사본에도 `node --check` 가 exit 0 을 돌려준다. 대신 이 둘 중 하나를 쓴다:
  - `node --input-type=module --check < js/game.js`
  - `node --experimental-vm-modules` + `vm.SourceTextModule` 로 파싱

  `js/game.js` 에는 유닛 테스트가 없다. 파싱 확인이 유일한 자동 그물이다.
...
| `sql/setup/orchard_events.sql` | `orchard_events` 테이블 DDL | 신설 |
...
> **목재 인플레는 구조적으로 불가능하다.** 숲 나무는 30초마다(`TREE_RESPAWN_SEC`) 목재 3개를 공짜로 준다. 과수원 나무는 최소 90🪙를 치러야 최대 3개다 — 목재 1개당 30🪙 대 0🪙다. 단계 비례 보상은 그 위에 얹는 두 번째 자물쇠로, 심자마자 베면 1개뿐이라 묘목을 사고 베는 짓이 더 무의미해진다.

`hp` 는 런타임 전용이다 — 세이브에 넣지 않는다(반쯤 팬 밭의 `digAt` 과 같은 취급). 과수원을 나갔다 오면 온전한 나무로 돌아온다.

액션 라우팅에도 등록한다 — 🪓도끼를 들고 나무 앞이면 베기가 먼저다:

```js
if (atOrchard && TOOLS[currentTool].id === 'axe') { const t = orchardTreeNear(); if (t) return chopTree(t); }
```
...
**Files:**
- Create: `sql/setup/orchard_events.sql`, `functions/api/orchard-events.js`
...
```bash
git add sql/setup/orchard_events.sql functions/api/orchard-events.js worker/index.js
git commit -m "$(cat <<'EOF'
...
`orchard_unlock` · `orchard_enter` · `sapling_buy` · `sapling_plant` · `tree_mature` · `tree_water` · `fruit_ready` · `fruit_harvest` · `fruit_capped` · `tree_chop`

`tree_chop` 은 나무를 하나 심어 바로 베어서 확인한다(묘목 단계라 목재 1개). `stage` 파라미터가 실려야 "갈아타기"와 "잘못 심음"을 나중에 나눌 수 있다.
