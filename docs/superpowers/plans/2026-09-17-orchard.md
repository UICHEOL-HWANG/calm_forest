...
`spawnTree()`(`js/game.js:2608`)를 본뜨되 **공유 지오메트리를 그대로 재사용**한다. 그루당 드로우콜 2를 유지하는 게 핵심이다.

```js
// 🍎 과일나무 — 기존 숲 나무와 같은 줄기·잎덩이 지오메트리를 쓰고 색만 다르게.
//   공유 리소스를 그대로 쓰므로 그루당 드로우콜은 숲 나무와 같은 2다.
function spawnFruitTree(x, z, kind, stage, fruit) {
  const def = FRUITS.find(f => f.id === kind) || FRUITS[0];
  const tree = new THREE.Group();
  tree.position.set(x, 0, z);
  const s = stage === 'sapling' ? 0.35 : stage === 'growing' ? 0.7 : 1;

  const trunk = new THREE.Mesh(
    shared('tree.trunk.geo', () => new THREE.CylinderGeometry(0.35, 0.5, 1.6, 7)),
    shared('tree.trunk.mat', () => clayMat(PAL.trunk)));
  trunk.position.y = 0.8 * s; trunk.scale.setScalar(s); trunk.castShadow = true; tree.add(trunk);
  ...
  canopy.position.y = 2.0 * s; canopy.scale.setScalar(s); canopy.castShadow = true; tree.add(canopy);
  canopy.userData.swayPhase = Math.random() * Math.PI * 2; swayables.push(canopy);

  // 열매 — 달린 개수만큼만 보인다. 색만 다른 공유 구체라 드로우콜이 안 늘어난다
  if (stage === 'mature' && fruit > 0) {
    const spots = [[-0.7, 2.2, 0.3], [0.8, 2.0, -0.2], [0.2, 2.6, 0.5], [-0.3, 1.8, -0.7], [0.6, 2.5, 0.1], [-0.9, 2.4, -0.3]];
    for (let i = 0; i < Math.min(fruit, spots.length); i++) {
      const b = new THREE.Mesh(
        shared('orchard.fruit.geo', () => new THREE.IcosahedronGeometry(0.17, 0)),
        shared(`orchard.fruit.mat.${def.fruitColor}`, () => clayMat(def.fruitColor)));
      b.position.set(spots[i][0], spots[i][1], spots[i][2]); tree.add(b);
    }
  }
  tree.userData = { kind, stage, collider: solidCircle(x, z, 0.8) };
  orchardGroup.add(tree);
  return tree;
}
...
function rebuildOrchard() {
  ...
  buildOrchardGround();     // 지면·오솔길·시냇물(정적)
  for (const t of (gameState.orchard?.trees || [])) spawnFruitTree(t.x, t.z, t.kind, t.stage, t.fruit);
  syncOrchardSlotHints();   // 빈 자리 표시 — 밭의 seedHint 와 같은 꼴
}
```

**이 두 헬퍼도 이 태스크에서 같이 만든다.** 이름만 부르고 끝내면 `ReferenceError` 가 난다.

```js
// 언덕 지면 · 오솔길 · 시냇물 — 정적이라 rebuild 마다 새로 그려도 싸다
function buildOrchardGround() {
  ...
  // 시냇물 — 중심선 점을 이어 납작한 판으로. nearStream 이 보는 좌표와 같은 값을 쓴다
  for (const p of orchardStreamWorld()) {
    const w = new THREE.Mesh(
      shared('orchard.water.geo', () => new THREE.CircleGeometry(2.2, 12).rotateX(-Math.PI / 2)),
      shared('orchard.water.mat', () => clayMat(0x8fb9d6)));
    w.position.set(p.x, 0.03, p.z); orchardGroup.add(w);
  }
}

// 빈 자리 표시 — 나무가 없는 흙 자리에만 낮은 원반을 둔다
function syncOrchardSlotHints() {
  const taken = (gameState.orchard?.trees || []).map(t => `${t.x},${t.z}`);
  for (const s of orchardSlotsWorld()) {
    if (taken.includes(`${s.x},${s.z}`)) continue;
    const m = new THREE.Mesh(
      shared('orchard.slot.geo', () => new THREE.CircleGeometry(0.9, 12).rotateX(-Math.PI / 2)),
      shared('orchard.slot.mat', () => clayMat(0x8a6440)));
    m.position.set(s.x, 0.04, s.z); orchardGroup.add(m);
  }
}
```
...
Run: 과수원에서 개발 콘솔에 `renderer.info.render.calls`
Expected: 나무 10그루 기준으로 **숲 나무 10그루와 같은 수준**(그루당 2 + 지형). 그루당 3 이상이면 `shared()` 키가 어긋나 재질이 그루마다 새로 만들어진 것이다 — `orchard.leaf.mat.${color}` 처럼 **색을 키에 넣었는지** 확인한다.
