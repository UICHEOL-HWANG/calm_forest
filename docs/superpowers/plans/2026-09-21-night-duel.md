...
- [ ] **Step 1: 후보를 표로 만들어 제시한다**

자리마다 후보를 2~3개씩 낸다. 확정 전에는 코드에 쓰지 않는다.

| 자리 | 후보 예 |
|---|---|
| 대결 시작 배너 (🐗) | "멧돼지가 길을 막아섰어요" / "멧돼지와 눈이 마주쳤어요" |
| 대결 시작 배너 (🦝) | "너구리가 바가지 셋을 늘어놨어요" / "너구리가 장난을 걸어왔어요" |
| 가위바위보 세 버튼 | ✊ / ✌️ / 🖐️ — 이모지만 둘지, "바위·가위·보" 라벨을 붙일지 |
| 판 표시 | "1 / 3" / "첫판" |
| 비겼을 때 | "비겼어요! 다시" |
| 그릇 안내 | "어느 바가지에 있을까요?" |
| 승리 (🐗) | "되찾았어요! 당분간 안 올 거예요" |
| 패배 | "놓쳤어요… 내일 다시 만나요" |
| 휴전 밤 토스트 | "🤝 어제 이긴 숲 친구가 약속을 지켜 오지 않았어요" |

- [ ] **Step 2: 모바일 폭을 실측한다**

가장 긴 후보가 375px 폭에서 두 줄로 넘어가지 않는지 본다. 넘어가면 짧은 후보로 바꾼다. **추정하지 말고 실제로 띄워서 잰다.**

- [ ] **Step 3: 확정안을 이 문서에 적는다**

이 태스크 자리에 확정 문구 표를 덧쓴다. Task 8·10 이 이 표를 그대로 쓴다.

```bash
git add docs/superpowers/plans/2026-09-21-night-duel.md
git commit -m "docs: 🐗🦝 대결 문구 확정"
```
