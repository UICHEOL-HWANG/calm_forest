...
- [ ] ⚠️ `wrangler.jsonc` 의 `triggers.crons` 는 **배열에 추가**한다. 지금 카드뉴스 크론
      `"0 2 * * *"` 이 들어 있어 덮어쓰면 카드뉴스가 죽는다. `scheduled` 핸들러도
      이미 있으므로(`worker/index.js:70`) `event.cron` 으로 분기한다
- [ ] `wrangler.jsonc` 에 `observability.enabled`
- [ ] 크론 실패 시 기존 `send_email` 바인딩 `NOTIFY` 로 메일 1통(시크릿 0개 — 이미 있는 채널)
