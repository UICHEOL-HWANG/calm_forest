...
def test_sample_size_near_measured():
    """표본이 실측 범위를 크게 벗어나면 조인이나 필터가 바뀐 것이다.

    ⚠️ 이 표본은 **자란다** — 베타 세션이 쌓이면 건수가 느는 게 정상이다.
       실측: 2026-09-04 = 507건 · 2026-09-06 = 724건 (이틀에 +43%).
       그래서 상한을 좁게 잡으면 아무 문제가 없어도 며칠마다 빨개진다.
       여기서 잡으려는 건 '자라는 것' 이 아니라 '조인이 끊겨 반토막' ·
