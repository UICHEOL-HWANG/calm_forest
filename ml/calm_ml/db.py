# =============================================================
#  calm forest · Supabase(Postgres) 조회 모듈 — 실시간 운영 조회용
#  ------------------------------------------------------------
#  ⚠️ 분석·학습에는 쓰지 말 것. 여기는 bq.py 가 담당한다.
#     Supabase 는 최근 7일만 보관한다(export_to_bq.py 가 적재 후 prune).
#     이 모듈의 load_d1_cohorts()/load_daily_active() 같은 헬퍼로 리텐션을 재면
#     7일 밖 이력이 통째로 빠진 채 조용히 숫자가 나온다 — 오류가 아니라 오답이라 더 위험하다.
#     "지금 로그가 쌓이고 있나" 같은 실시간 점검에만 쓴다.
