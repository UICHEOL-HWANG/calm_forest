...
- ⚠️ 바다터 머리말 하나 아래 4,100줄(실내·주방·조각·프롤로그·상점·요리·장식·텃밭·방문객·동굴·문)이 섞여 있었다 → 주석 머리말만 먼저 넣고(verify-extract 가 주석 무시) 나눴다
- ⚠️ i18n_check 는 읽을 파일 목록이 하드코딩 — 새 디렉터리를 만들면 목록에 넣어야 번역 누락을 잡는다
- ⚠️ 브라우저 창이 가려지면 1.5fps — 컷신이 dt 누적이라 멈춘 듯 보인다. 흐름 점검은 헤드리스(flows.mjs)로
- ⚠️ 훅이 같은 명령 줄의 `grep -n` 을 `git commit -n`(검사 건너뛰기)으로 오인해 막는다 — 커밋은 따로 실행
