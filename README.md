...
맵의 반투명 집 터에서 **망치**로 목재 10개씩 3단계(데크→통나무벽→지붕) 건설. 완성하면 문으로 **실내 입장** → 하단 **🎨 꾸미기**로 러그·화분·의자·테이블·램프·소파·어항(+큰 가구)을 작물/물고기로 사서 배치. 고르면 바닥에 **반투명 미리보기(초록 링)**가 뜨고, 모바일은 바닥 탭으로 자리를 잡은 뒤 오른쪽 가구 버튼(또는 같은 자리 한 번 더 탭)으로, 데스크톱은 마우스로 자리를 정해 클릭으로 놓습니다. **놓은 가구는 탭/클릭하면 다시 들어 옮길 수 있고**(비용 없음), 첫 방문 땐 ①~④ 단계 안내가 뜹니다. 밤엔 방 안이 은은하게 밝아집니다.
...
- **정화(데일리)**: 수호목 앞에서 시작 → 정령 세 무리(3·4·5마리, 안개 날 +1)가 차례로 다가옴. 전부 달래면 그날은 안개가 걷힌 맑은 숲(매일 자정 리셋).
- **🎓 연습 모드(2026-09-07)**: 수호목 앞 액션 → 첫 방문엔 `🌳 바로 시작 / 🎓 연습해 보기` 갈림길 카드(연습 1회 또는 정화 1회 후엔 바로 시작). 연습은 정령 1마리·빛 안 줄어듦·보상/도감/기록 없음, 컨텍스트 슬롯에 4단계 안내. 첫 실패 후 연습 미경험이면 "조금 어려웠나요?" 모달로 한 번 더 제안.
